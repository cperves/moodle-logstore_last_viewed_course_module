...
class restore_logstore_last_viewed_course_module_subplugin extends restore_tool_log_logstore_subplugin {
    ...
    protected function define_logstore_subplugin_structure() {
        ...
        if (!in_array('logstore_last_viewed_course_module', $enabledlogstores)) {
        ...
        $elepath = $this->get_pathfor('/logstore_lastviewed_log');
     ...
     */
    public function process_logstore_last_viewed_course_module_log($data) {
        global $DB;
        $contextrecord = $DB->get_record('context', array('instanceid' => $data['cmid'], 'contextlevel' => CONTEXT_MODULE));
        if ($contextrecord) {
        ...
        if ($data) {
            ...
            if ($data->contextlevel == CONTEXT_MODULE) {
                ...
                $newdata->cmid = $data->contextinstanceid;
                $newdata->lasttimeviewed = $data->lasttimeviewed;
                $newdata->userid = $data->userid;
                $DB->insert_record('logstore_lastviewed_log', $newdata);
            }
        }
    }
}
