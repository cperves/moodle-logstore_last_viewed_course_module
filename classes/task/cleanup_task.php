...
namespace logstore_last_viewed_course_module\task;
...
class cleanup_task extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('taskcleanup', 'logstore_last_viewed_course_module');
    }
    ...
    public function execute() {
        ...
        $loglifetime = (int)get_config('logstore_last_viewed_course_module', 'loglifetime');
        ...
        while ($min = $DB->get_field_select("logstore_lastviewed_log", "MIN(lasttimeviewed)", "lasttimeviewed < ?", $lifetimep)) {
            ...
            $DB->delete_records_select("logstore_lastviewed_log", "lasttimeviewed < ?", $params);
            if (time() > $start + 300) {
        ...
        mtrace(" Deleted old log records from last_viewed_course_module log store.");
    }
}
